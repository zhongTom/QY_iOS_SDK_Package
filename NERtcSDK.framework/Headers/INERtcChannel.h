 ...
 * @par 示例代码
 * @code
 * int result = [[NERtcEngine sharedEngine] joinChannelWithToken:token completion:^(NSError * _Nullable error, uint64_t channelId, uint64_t elapesd, uint64_t uid) {
 *                 if (error) {
 *                    //join room failed
 *                 }
 *                 else {
 ...
 *          - 30403（kNERtcErrMediaOpenBannedByServer）：被服务器禁言。
 * @endif
 */
...
* @note 
* - 该方法仅适用于视频主流，若您希望为辅流通道设置摄像头的采集配置，请调用 {@link INERtcChannel#setCameraCaptureConfig:streamType:}。
 ...
 * 取消或恢复发布本地视频流。
 * @note
 * - 该方法仅适用于视频主流，若您希望取消或恢复发布视频辅流，请调用 {@link INERtcChannel#muteLocalVideo:streamType:} 方法。
 ...
 *      <td>volume</td>
 *      <td>uint64_t</td>
 *      <td>播放音量，取值范围为 [0,100]。<ul><li>0：静音。<li>100：原始音量。</td>
 *  </tr>
 * </table>
 * @par 示例代码
 * @code
 ...
 * @par 示例代码
 * @code
 * //getPermToken为获取高级密钥方法，由您自行实现，具体请参考<a href="https://doc.yunxin.163.com/jcyOTA0ODM/docs/TU3NjMyOTc?platform=iOS" target="_blank">高级 Token 鉴权</a>
 * [self getToken];
