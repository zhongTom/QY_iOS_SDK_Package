 ...
 * @if Chinese
 * 开启美颜功能模块。
 * - 调用此接口后，开启美颜引擎。如果后续不再需要使用美颜功能，可以调用 {@link NERtcBeauty#stopBeauty} 结束美颜功能模块，销毁美颜引擎并释放资源。
 ...
 * @if Chinese
 * 结束美颜功能模块。
 * <br>如果后续不再需要使用美颜功能，可以调用 {@link NERtcBeauty#stopBeauty} 结束美颜功能模块，SDK 会自动销毁美颜引擎并释放资源。
 * @since V4.2.202
 * @return
 * - 0：方法调用成功。
 * - 其他：方法调用失败。
 ...
 * @note 
 * - 使用滤镜、贴纸和美妆等自定义美颜效果之前，需要先通过 {@link NERtcBeauty#startBeauty} 开启美颜模块。
