 ...
 * @endif
 */
NERTC_EXPORT  @interface NERtcTextWatermarkConfig : NSObject
/**
 * @if English
 * Text content. If the value is set to empty, no text watermark is added.
...
/**
 * @if English
 * Specifies the font size. The default value is 17.
 * @endif
 * @if Chinese
 * 字体大小。默认值为 17。
 * @endif
...
/**
 * @if English
 * Specifies the font color. ARGB format. The default value is 0xFFFFFFFF, which is white.
 * @endif
 * @if Chinese
...
/**
 * @if English
 * The background color in the watermark frame. ARGB format. The default value is 0x88888888, which is gray. Transparency setting is supported.
 * @endif
 * @if Chinese
...
/**
 * @if English
 * The frame layout of the image. If the width or height is not set, or both are set to 0, the watermark is inserted based on the original image size.
 * @endif
 * @if Chinese
 ...
 * @if English
 * Timestamp watermark
 * - You can add only 1 timestamp watermark that follows the yyyy-MM-dd Hh:mm:ss format.
 ...
 * @endif
 */
NERTC_EXPORT  @interface NERtcTimestampWatermarkConfig : NSObject
 ...
 * <br>Supports 4 image watermarks.
 * @endif
 * @if Chinese
 * 图片水印设置参数。
 * <br>支持设置 4 个图片水印。
 * @endif
 */
NERTC_EXPORT  @interface NERtcImageWatermarkConfig : NSObject
...
/**
 * @if English
 * Specifies the frame rate. The default value is 0 fps, which indicates that the images are not flipped automatically. Images are displayed in static single frames.
 * @endif
 * @if Chinese
 ...
 * <br>You can set text, timestamp, and image watermarks at the same time. If different types of watermarks overlap, the layers overlay previous layers in the image, text, and timestamp sequence.
 * @endif
 * @if Chinese
 * 画布水印设置。
 ...
 * @endif
 */
NERTC_EXPORT  @interface NERtcCanvasWatermarkConfig : NSObject

/**
 * @if English
 * Text watermark You can add up to 10 text watermarks.
 * @endif
 * @if Chinese
...
/**
 * @if English
 * Timestamp watermark You can add only 1 timestamp watermark.
 * @endif
 * @if Chinese
 * 时间戳水印。只能添加 1 个时间戳水印。
 * @endif
...
/**
 * @if English
 * Image watermarks. You can add up to 4 picture watermarks.
 * @endif
 * @if Chinese
 * 图片水印，最多可以添加 4 个图片水印。
 * @endif
